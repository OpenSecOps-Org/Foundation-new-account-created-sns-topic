...
This component was converted to the locked-dependency model in **v1.2.8**. The supply-chain posture below applies to v1.2.8 and every release after it. Earlier releases predate the conversion and do not carry these guarantees; per §1, the remediation for any exposure on an earlier release is to upgrade to the current release.
...
```
https://github.com/OpenSecOps-Org/Foundation-new-account-created-sns-topic/releases/download/<VERSION>/Foundation-new-account-created-sns-topic-<VERSION>-sbom.cdx.json
```

Replace `<VERSION>` with the release tag (e.g. `v1.2.8`). The asset is attached to every release on the public OpenSecOps-Org remote.
...
Together they are the authoritative cross-component references; the per-component sections above describe how the project-wide posture applies to **Foundation-new-account-created-sns-topic** specifically.
